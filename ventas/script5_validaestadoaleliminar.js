...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (modifyForm) {
        modifyForm.addEventListener('submit', function(event) {
            ...
            sales[index].status = 'Pendiente';  // Cambiar el estado a "Pendiente" al modificar
            localStorage.setItem('sales', JSON.stringify(sales));
        ...
        if (params.has('index')) {
            ...
            if (sale.status === 'Procesado') {
                alert("No se puede modificar este producto porque ya se encuentra procesado.");
...
function displaySalesList(container) {
    ...
    sales.forEach((sale, index) => {
        ...
        saleDiv.innerHTML = `
            ...
            <button onclick="editSale(${index})" ${sale.status === 'Procesado' ? 'disabled' : ''}>Modificar</button>
